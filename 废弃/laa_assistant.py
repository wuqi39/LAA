...
# 定义资源文件根目�?ROOT_RESOURCE = os.path.join(os.path.dirname(__file__), 'resource')
...
class ViewTasksTool(BaseTool):
    ...
    parameters = [{
        'name': 'status',
        'type': 'string',
        'description': '任务状�?(pending/completed/all)',
        'required': False
    }]
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        for task in tasks:
            if len(task) == 4:
                result += f"ID: {task[0]}, 标题: {task[1]}, 状�? {task[3]}, 创建时间: {task[2]}\n"
            else:
                result += f"ID: {task[0]}, 标题: {task[1]}, 描述: {task[2]}, 状�? {task[3]}, 创建时间: {task[4]}\n"
...
class UpdateTaskTool(BaseTool):
    ...
    parameters = [{
        ...
        'name': 'status',
        'type': 'string',
        'description': '新状�?(pending/completed)',
        'required': False
    }, {
        'name': 'title',
        'type': 'string',
...
class WeatherTool(BaseTool):
    ...
    def get_weather_from_gaode(self, location: str) -> str:
        """调用高德地图API查询天气"""
        # 从环境变量获取高德API Key，如果未设置则提示用�?        gaode_api_key = os.getenv('AMAP_API_KEY', '')
        if not gaode_api_key or gaode_api_key == '':
            return "请设置环境变�?AMAP_API_KEY 以使用天气查询功�?
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('status') == '1' and data.get('lives'):
                    ...
                    # 检查时间格式是否完整（应为 YYYY-MM-DD HH:MM:SS 格式�?                    if report_time and len(report_time) >= 16 and report_time[-1].isdigit():
                        # 时间格式完整
...
class BingSearchTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            # 处理MCP服务返回的结�?            if result.get('status') == 'success':
                # 在Trae AI环境中，run_mcp会自动被替换为真实的服务调用
                # 我们直接返回结果，让系统处理实际的调�?                if 'is_mcp_request' in result.get('data', {}):
                    # 这是在模拟环境中，我们构造一个示例响�?                    formatted_result = f"已发送搜索请求到Bing MCP服务 (关于 '{query}')。\\n"
                    ...
                    # 处理真实的搜索结�?                    search_results = result.get('data', {}).get('results', [])
                    if search_results:
                        formatted_result = f"搜索结果 (关于 '{query}')：\\n\\n"
                        for i, item in enumerate(search_results, 1):
                            formatted_result += f"{i}. **{item.get('title', '未知标题')}**\\n"
                            formatted_result += f"   描述: {item.get('description', '无描�?)}\\n"
                            formatted_result += f"   链接: {item.get('url', '无链�?)}\\n\\n"
                        return formatted_result
                    else:
                        return f"未找到关�?'{query}' 的信�?
            else:
...
class SearchAttractionsTool(BaseTool):
    """
    景点搜索工具，使用高德地图MCP服务搜索指定地点的景点信息，并提供景点图片链�?    """
    description = '搜索指定地点的景点信�?
    parameters = [{
        'name': 'location',
        'type': 'string',
        'description': '搜索的地点名称，例如：北京、上海、杭州西�?,
        'required': True
    }]
        ...
        try:
            ...
            # 处理MCP服务返回的结�?            if result.get('status') == 'success':
                ...
                if 'is_mcp_request' in result.get('data', {}):
                    ...
                    # 处理真实的搜索结�?                    attractions_data = result.get('data', {}).get('results', [])
                    
                    if attractions_data:
                        result_text = f"{location}的热门景点信息：\n\n"
                        
                        # 处理�?个景�?                        for i, attraction in enumerate(attractions_data[:5], 1):
                            ...
                            if detail_result and detail_result.get('status') == 'success':
                                ...
                                if detail_data and 'photos' in detail_data:
                                    photos = detail_data['photos'][:2]  # 最多显�?张图�?                                    for j, photo in enumerate(photos, 1):
                                        photo_url = photo.get('url', '')
                                        if photo_url:
                                            # 使用Markdown图片格式，以便WebUI组件能正确渲�?                                            result_text += f"   图片{j}: ![{name}图片{j}]({photo_url})\n"
                                        else:
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            if analysis_type == 'summary':
                # 基本统计汇�?                numeric_cols = df.select_dtypes(include=[np.number]).columns
                ...
                if len(numeric_cols) > 0:
                    for col in numeric_cols:
                        result += f"{col}的分布统计：\n"
                        result += f"  均�? {df[col].mean():.2f}\n"
                        result += f"  中位�? {df[col].median():.2f}\n"
                        result += f"  标准�? {df[col].std():.2f}\n\n"
                else:
                ...
                if len(numeric_cols) > 0:
                    for col in numeric_cols:
                        result += f"{col}列：\n"
                        result += f"  最大�? {df[col].max():.2f}\n"
                        result += f"  最小�? {df[col].min():.2f}\n"
            ...
            try:
                ...
                if 'value' in df.columns:
                    ...
                    if chart_type == 'bar':
                        ...
                        # 对于散点图，需要两列数�?                        if len(df.columns) >= 2 and all(col in df.columns for col in ['x', 'y']):
...
class MCPFetchTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            # 调用MCP fetch服务
            from laa_assistant import run_mcp  # 临时导入，避免循环依�?            result = run_mcp(
                server_name='fetch',
                tool_name='fetch',
...
class MCPWeatherTool(BaseTool):
    ...
    def format_weather_result(self, result: dict) -> str:
        ...
        # 根据实际返回结构格式化输�?        if isinstance(result, dict):
...
class MCPTrainTicketTool(BaseTool):
    ...
    parameters = [{
        ...
        'type': 'string',
        'description': '出发城市或车站名�?,
        'required': True
    }, {
        'name': 'arrival_station',
        'type': 'string',
        'description': '到达城市或车站名�?,
        'required': True
    }, {
        'name': 'date',
        'type': 'string',
        ...
        'name': 'filter',
        'type': 'string',
        'description': '车次筛选条件，如G(高铁/城际),D(动车),Z(直达特快),T(特快),K(快�?�?,
        'required': False
    }]
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            # 调用MCP火车票查询服�?            from laa_assistant import run_mcp  # 临时导入，避免循环依�?            result = run_mcp(
                server_name='juhe-mcp-server',
    ...
    def format_train_result(self, result: dict) -> str:
        ...
        # 根据实际返回结构格式化输�?        if isinstance(result, dict):
            if 'tickets' in result:
                ...
                for ticket in tickets[:5]:  # 最多显�?条记�?                    result_str += f"车次：{ticket.get('train_no', 'N/A')} | 出发时间：{ticket.get('departure_time', 'N/A')} | 到达时间：{ticket.get('arrival_time', 'N/A')} | 价格：{ticket.get('price', 'N/A')}\n"
                return result_str
            else:
                return json.dumps(result, ensure_ascii=False, indent=2)
        return str(result)
...
class MCPMapsTool(BaseTool):
    ...
    """
    description = '使用地图服务查询地理位置信息或进行路径规�?
    parameters = [{
        'name': 'action',
        'type': 'string',
        'description': '操作类型：geocode(地址转坐�?、regeocode(坐标转地址)、direction_driving(驾车导航)、distance(距离测量)',
        'required': True
    }, {
        'name': 'address',
        'type': 'string',
        ...
        'name': 'location',
        'type': 'string',
        'description': '经纬度坐标，格式为经�?纬度，用于regeocode等操�?,
        'required': False
    }, {
        'name': 'origin',
        'type': 'string',
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            # 根据action选择不同的地图服�?            from laa_assistant import run_mcp  # 临时导入，避免循环依�?            
            if action == 'geocode':
                # 地址转坐�?                result = run_mcp(
                    server_name='amap-maps',
                    tool_name='maps_geo',
...
class ChartTool(BaseTool):
    ...
    parameters = [{
        ...
        'name': 'data',
        'type': 'array',
        'description': '图表数据，例�?[{"name": "项目A", "value": 30}, {"name": "项目B", "value": 70}]',
        'required': True
    }, {
        'name': 'title',
        'type': 'string',
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        for item in data:
            if isinstance(item, dict):
                if 'name' in item and 'value' in item:
                    ...
                    # 尝试直接从字典中获取�?                    for key, value in item.items():
        ...
        if chart_type == 'bar':
            ...
            ax.set_ylabel(y_label if y_label else '数�?)
        elif chart_type == 'line':
            ...
            plt.close(fig)
            return f"错误：不支持的图表类�?'{chart_type}'，支持的类型：bar, line, pie, scatter"
        ...
        # 将图表保存到内存中的字节�?        img_buffer = io.BytesIO()
        plt.savefig(img_buffer, format='png', dpi=150, bbox_inches='tight')
        img_buffer.seek(0)

        # 将图片转换为base64编码
        img_base64 = base64.b64encode(img_buffer.getvalue()).decode()
        plt.close(fig)  # 关闭图形以释放内�?
        # 创建一个临时文件保存图表（如果需要）
        import tempfile
        import os
        ...
        return f"图表生成成功！图表已保存�?{filepath}。图表类�? {chart_type}, 数据点数�? {len(data)}"
...
以下是可用的功能�?1. 任务管理：创建、查看、更新、删除个人任�?2. 笔记记录：记录和检索个人笔�?3. 天气查询：查询指定城市的天气信息
4. 网络搜索：获取网络信�?5. 图表生成：根据数据生成柱状图、折线图、饼图、散点图�?6. 景点搜索：搜索指定地点的景点信息，并提供景点图片链接
...
# 初始化助理服�?# MCP服务调用函数
    ...
    Args:
        server_name: MCP服务器名�?        tool_name: 工具名称
        args: 工具参数
    ...
    try:
        ...
        if server_name not in available_servers:
            return {
                'status': 'error',
                'message': f'未知的MCP服务�? {server_name}',
        ...
        if server_name == "amap-maps":
            ...
            if amap_api_key:
                ...
                print("警告: 未设置环境变�?AMAP_API_KEY，将使用配置的默认值\")
        
        # 这里我们返回一个特殊的响应，表明我们已经识别到了MCP服务请求
        # 实际上，在Trae AI环境中，当调用这个函数时，系统会自动拦截并使用真正的run_mcp工具
        # 这种方式避免了在Python代码中尝试导入不存在的模�?        return {
            'status': 'success',
            'message': f'MCP服务调用请求已接�? {server_name}.{tool_name}',
            'data': {
...
class MCPAmapMapsTool(BaseTool):
    """
    高德地图MCP服务工具，支持地址查询、路径规划、景点搜索等
    """
    description = '使用高德地图服务进行地址查询、路径规划、景点搜索等'
    parameters = [{
        'name': 'action',
        'type': 'string',
        'description': '操作类型：geocode(地址转坐�?、regeocode(坐标转地址)、text_search(文本搜索)、direction_driving(驾车导航)、distance(距离测量)、weather(天气查询)、search_detail(搜索详情)',
        'required': True
    }, {
        'name': 'query',
        'type': 'string',
        'description': '搜索查询词，用于text_search操作',
        'required': False
    }, {
        'name': 'address',
        'type': 'string',
        ...
        'name': 'city',
        'type': 'string',
        'description': '城市名称，用于text_search或weather操作',
        'required': False
    }, {
        'name': 'id',
        'type': 'string',
        'description': 'POI ID，用于search_detail操作',
        'required': False
    }]
    ...
    def format_maps_result(self, action: str, result: dict) -> str:
        ...
        if isinstance(result, dict):
            if result.get('status') == 'success':
                ...
                if 'is_mcp_request' in data:
                    # 这是在模拟环境中
                    formatted_result = f"已发送{action}请求到高德地图MCP服务。\\n"
                    formatted_result += "在实际环境中，您将收到实际的搜索结果。\"
                    return formatted_result
                else:
                    # 处理真实结果
                    if action == 'text_search' and 'results' in data:
                        ...
                        if results:
                            formatted_result = f"搜索结果 (共{len(results)}�?:\\n\\n"
                            for i, item in enumerate(results[:5], 1):  # 最多显示前5个结�?                                name = item.get('name', '未知名称')
                                address = item.get('address', '地址未知')
                                location = item.get('location', '位置未知')
                                formatted_result += f"{i}. **{name}**\\n"
                                formatted_result += f"   地址: {address}\\n"
                                formatted_result += f"   位置: {location}\\n\\n"
                            return formatted_result
                        else:
                            return "未找到相关搜索结果\"
                    elif action == 'search_detail' and data:
                        # 处理POI详情
                        formatted_result = "地点详情:\\n"
                        for key, value in data.items():
                            if key not in ['is_mcp_request', 'note']:
                ...
                return f"地图服务调用失败：{result.get('message', '未知错误')}"
